...
export function fromSignal(signal, endTime, config = null) {
    ...
    let { height, width, strokeWidth, colors } = config;
    ...
    */
    ...
    let cutPath = (color, at=[0, height]) => {
        ...
        if (color) {
            let [x, y] = at;
            paths.push(`<path stroke="${color}" fill="transparent" stroke-width="${strokeWidth}" d="M ${x} ${y}`);
        }
    };
