...
export class Wavedash {
    ...
    async vcdParsed() {
        ...
        this._vcdParsed = parse(this.vcd);
    ...
    async render() {
        ...
        let { finalTime, timescale } = this.parsed;
        this.config.endTime = finalTime;

        let signals = [];
        let flattenSignals = (scope) => {
            for (let signal of scope.signals) {
                signals.push(signal);
            }
            for (let key in scope.hierarchy) {
                flattenSignals(scope.hierarchy[key])
            }
        };
        flattenSignals(this.parsed.top);
        ...
        function update(div, signal, t) {
            let trackedValue = "0";
            for (let [rawTime, value] of signal.waveform) {
